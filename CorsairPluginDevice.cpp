...
cue::dev::plugin::DeviceInfo* CorsairPluginDevice::CreateDeviceInfo()
{
	...
	// Assign unique device type per device based on index set at connect time.
	// CDT_Keyboard and CDT_Mouse support individual LED control in iCUE.
	if (mDeviceIndex == 0) {
		deviceInfo->deviceType = cue::dev::plugin::DeviceType::CDT_Keyboard;
	}
	else if (mDeviceIndex == 1) {
		deviceInfo->deviceType = cue::dev::plugin::DeviceType::CDT_Mouse;
	}
	else if (mDeviceIndex == 2) {
