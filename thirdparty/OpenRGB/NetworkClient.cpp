...
listen_done:
    ...
    {
        std::vector<RGBController *> server_controllers_copy_listen;
        ...
        server_controllers_copy_listen = server_controllers;
        ...
        /*-------------------------------------------------*\
        | Client info has changed, call the callbacks       |
        | Must happen BEFORE deleting controllers so that   |
        | DisconnectDevices() clears the device map first.  |
        \*-------------------------------------------------*/
        ClientInfoChanged();

        /*-------------------------------------------------*\
        | Safe to delete now - device map has been cleared  |
        \*-------------------------------------------------*/
        for(size_t server_controller_idx = 0; server_controller_idx < server_controllers_copy_listen.size(); server_controller_idx++)
        {
            delete server_controllers_copy_listen[server_controller_idx];
        }
    }
}
...
void NetworkClient::ProcessReply_ControllerData(unsigned int /*data_size*/, char* data, unsigned int dev_idx)
{
    ...
    {
        // Full sync so zone LED counts are updated after SetCustomMode
        server_controllers[dev_idx]->active_mode = new_controller->active_mode;
        server_controllers[dev_idx]->modes       = new_controller->modes;
        if (server_controllers[dev_idx]->zones.size() == new_controller->zones.size())
        {
            for (size_t z = 0; z < new_controller->zones.size(); z++)
            {
                server_controllers[dev_idx]->zones[z].leds_count = new_controller->zones[z].leds_count;
                server_controllers[dev_idx]->zones[z].start_idx  = new_controller->zones[z].start_idx;
            }
...
void NetworkClient::ProcessRequest_DeviceListChanged()
{
    ...
    server_controllers_copy = server_controllers;
    server_controllers.clear();
